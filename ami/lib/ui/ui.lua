...
function Manager:init()
	...
	local handlers = {
		...
		mouse_up = function(_, button, x, y)
			...
			-- Vendored version: multishell debug feature removed
			-- if ie.code == 'control-shift-mouse_click' then ... end
...
local function loadComponents()
	local function load(name)
		...
		local env = setmetatable({ UI = UI }, { __index = _ENV })
		...
		if not ok then
			error('Error executing component ' .. name .. ': ' .. tostring(execErr))
		end
	...
	-- Vendored version: hardcoded component list
	local components = {
		'ActiveLayer', 'Button', 'Checkbox', 'Chooser', 'Dialog', 
		'DropMenu', 'DropMenuItem', 'Embedded', 'Form', 'Grid', 
		'Image', 'Menu', 'MenuBar', 'MenuItem', 'NftImage', 
		'Notification', 'ProgressBar', 'ScrollBar', 'ScrollingGrid', 
		'SlideOut', 'Slider', 'StatusBar', 'Tab', 'TabBar', 
		'TabBarMenuItem', 'Tabs', 'Text', 'TextArea', 'TextEntry', 
		'Throttle', 'TitleBar', 'VerticalMeter', 'Viewport', 
		'Wizard', 'WizardPage'
	}
...
loadComponents()
-- Vendored version: skip theme loading, use defaults
-- UI:loadTheme('usr/config/ui.theme')
Util.merge(UI.Window.defaults, UI.theme.Window or {})
Util.merge(UI.Page.defaults, UI.theme.Page or {})
