...
-- Register the address on the node (call once after key creation).
function comms.register(secretKey, nodeKey, address)
    return send(secretKey, nodeKey, { cmd="REGISTER", from=address }, true)
end
